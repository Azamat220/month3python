...
from data_base.bot_dp import sql_command_insert
...
async def fsm_start(message: types.Message):
    if message.chat.type == "private":
        # and message.chat.id in ADMINS
        await FSMAdmin.name.set()
        await message.answer("Как тебя зовут?", reply_markup=client_kb.cancel_markup)
    else:
...
async def load_name(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await message.answer("Сколько тебе лет?", reply_markup=client_kb.cancel_markup)
...
async def load_age(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        ...
        await FSMAdmin.next()
        await message.answer("Пол?", reply_markup=client_kb.gender_markup)
...
async def load_group(message: types.Message, state: FSMContext):
    ...
    await FSMAdmin.next()
    await message.answer("Твоё фото", reply_markup=client_kb.cancel_markup)
...
def register_handlers_fsm_mentor_anketa(dp: Dispatcher):
